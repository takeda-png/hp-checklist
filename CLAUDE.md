...
- **自動判定項目（21項目）**:
  ...
  **リンク・ボタン（2項目）:**
  8. `リンク切れの可能性` - 有効なリンク率確認
  9. `外部リンクがtarget="_blank"` - 外部リンクの新規タブ設定確認

  **デザイン（2項目）:**
  10. `ファビコン` - ブランドアイコン設定確認
  11. `viewportメタタグ` - レスポンシブ対応確認

  **パフォーマンス・技術（5項目）:**
  12. `HTTPS` - セキュアプロトコル対応確認
  13. `robots.txt` - robots.txtファイルの存在確認
  14. `sitemap.xml` - sitemap.xmlの存在確認
  15. `Google Analytics` - GA タグの設置確認
  16. `Google Tag Manager` - GTM タグの設置確認
  17. `charsetメタタグ` - 文字コード設定確認

  **法的・その他（3項目）:**
  18. `プライバシーポリシー` - プライバシーポリシーページの存在確認
  19. `コピーライト表記` - 著作権表記の有無確認
  20. `お問い合わせフォーム` - コンタクトフォームの存在確認
...
- [x] 21項目の自動判定機能（HTML要素検出、タグチェック、リンク検証）
